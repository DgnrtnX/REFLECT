...
const logger = log4js.getLogger('Reflect');
...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        if (fcn === "CreateAsset"){
            ...
            message = `Successfully added the Asset with ID ${args[0]}`

        } else if (fcn === "TransferAsset") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully changed asset owner with ID ${args[0]}`
            ...
            return `Invocation require either CreateAsset or TransferOwner as function but got ${fcn}`
        }
