export CORE_PEER_TLS_ENABLED=true
export ORDERER_CA=$REFLECT/hyperledger/crypto-config/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
export PEER0_ORG1_CA=$REFLECT/hyperledger/crypto-config/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt
export PEER0_ORG2_CA=$REFLECT/hyperledger/crypto-config/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt
export FABRIC_CFG_PATH=$REFLECT/docker/

export CHANNEL_NAME="supplychainchannel"

setGlobalsForPeer0Org1(){
    export CORE_PEER_LOCALMSPID="Org1MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG1_CA
    export CORE_PEER_MSPCONFIGPATH=$REFLECT/hyperledger/crypto-config/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
    export CORE_PEER_ADDRESS=localhost:7051
}

setGlobalsForPeer1Org1(){
    export CORE_PEER_LOCALMSPID="Org1MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG1_CA
    export CORE_PEER_MSPCONFIGPATH=$REFLECT/hyperledger/crypto-config/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
    export CORE_PEER_ADDRESS=localhost:8051
    
}

setGlobalsForPeer0Org2(){
    export CORE_PEER_LOCALMSPID="Org2MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG2_CA
    export CORE_PEER_MSPCONFIGPATH=$REFLECT/hyperledger/crypto-config/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
    export CORE_PEER_ADDRESS=localhost:9051
    
}

setGlobalsForPeer1Org2(){
    export CORE_PEER_LOCALMSPID="Org2MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG2_CA
    export CORE_PEER_MSPCONFIGPATH=$REFLECT/hyperledger/crypto-config/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
    export CORE_PEER_ADDRESS=localhost:10051
    
}

createChannel(){
    setGlobalsForPeer0Org1
    
    peer channel create -o localhost:7050 -c $CHANNEL_NAME --ordererTLSHostnameOverride orderer.example.com -f ../hyperledger/transactions/${CHANNEL_NAME}.tx --outputBlock ../hyperledger/blocks/${CHANNEL_NAME}.block --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA

    if [ "$?" -ne 0 ]; then
    echo "Failed to create channel!"
    exit 1
fi
}


joinChannels(){
    setGlobalsForPeer0Org1
    peer channel join -b ../hyperledger/blocks/$CHANNEL_NAME.block

    if [ "$?" -ne 0 ]; then
    echo "Failed to join channel: Peer0Org1"
    exit 1
    fi
    
    setGlobalsForPeer1Org1
    peer channel join -b ../hyperledger/blocks/$CHANNEL_NAME.block
    
    if [ "$?" -ne 0 ]; then
    echo "Failed to join channel: Peer1Org1"
    exit 1
    fi

    setGlobalsForPeer0Org2
    peer channel join -b ../hyperledger/blocks/$CHANNEL_NAME.block

    if [ "$?" -ne 0 ]; then
    echo "Failed to join channel: Peer0Org2"
    exit 1
    fi
    
    setGlobalsForPeer1Org2
    peer channel join -b ../hyperledger/blocks/$CHANNEL_NAME.block

    if [ "$?" -ne 0 ]; then
    echo "Failed to join channel: Peer1Org2"
    exit 1
    fi
    
}

updateAnchorPeers(){
    setGlobalsForPeer0Org1
    peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com \
    -c $CHANNEL_NAME -f ../hyperledger/transactions/${CORE_PEER_LOCALMSPID}anchors.tx \
    --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA

    if [ "$?" -ne 0 ]; then
    echo "Failed to update anchor peer: Peer0Org1"
    exit 1
    fi
    
    setGlobalsForPeer0Org2
    peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com \
    -c $CHANNEL_NAME -f ../hyperledger/transactions/${CORE_PEER_LOCALMSPID}anchors.tx \
    --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA

    if [ "$?" -ne 0 ]; then
    echo "Failed to update anchor peer: Peer0Org2"
    exit 1
    fi
    
}

createChannel
joinChannels
updateAnchorPeers